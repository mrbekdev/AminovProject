...
const SalesManagement = ({ selectedBranchId }) => {
  ...
  const generateReceipt = () => {
    ...
    fetchWithAuth(`${API_BASE_URL}/api/receipts`, {
      ...
      .then(() => {
        toast.success("Чек муваффақиятли сақланди!");
      })
      .catch((err) => {
        toast.error("Чекни сақлашда хатолик: " + err.message);
      });
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <p>Сана: ${new Date(receipt.date).toLocaleString("uz-UZ")}</p>
          <p>Кассир: ${receipt.cashier}</p>
          <p>Мижоз: ${receipt.customer}</p>
          <p>Етказиб бериш усули: ${
          ...
          <table>
            ...
            ${receipt.items
              ...
              <tr>
                <td>${item.name}</td>
                <td>${item.price.toLocaleString()} сўм</td>
                <td>${item.quantity}</td>
                <td>${item.total.toLocaleString()} сўм</td>
                ${receipt.paymentMethod === "credit" ? `
                  <td>${item.creditMonth}</td>
                  <td>${item.creditPercent}</td>
                  <td>${item.monthlyPayment.toLocaleString()} сўм</td>
                ` : ""}
              </tr>
            `
              )
          ...
          <p class="total">Жами: ${receipt.total.toLocaleString()} сўм</p>
          ${receipt.paymentMethod === "credit" ? `
            <p class="total">Фоиз билан жами: ${receipt.creditTotal.toLocaleString()} сўм</p>
          ` : ""}
          <p class="total">Тўланган сумма: ${receipt.amountPaid.toLocaleString()} сўм</p>
          <p class="total">Қолган сумма: ${receipt.remainingBalance.toLocaleString()} сўм</p>
          <p>Қайтариш коди: ${receipt.returnCode}</p>
        </body>
      </html>
    `);
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`Чек #${receipt.id}`, 10, 10);
    doc.setFontSize(12);
    doc.text(`Сана: ${new Date(receipt.date).toLocaleString("uz-UZ")}`, 10, 20);
    doc.text(`Кассир: ${receipt.cashier}`, 10, 30);
    doc.text(`Мижоз: ${receipt.customer}`, 10, 40);
    doc.text(
      `Етказиб бериш усули: ${
        receipt.deliveryMethod === "delivery"
          ? "Етказиб бериш"
    ...
    doc.text("Маҳсулотлар:", 10, 70);
    let y = 80;
    doc.setFontSize(10);
    let header = "Маҳсулот  Нарх  Сони  Жами";
    if (receipt.paymentMethod === "credit") {
      header += "  Ойлар  Фоиз  Ойлик тўлов";
    }
    ...
    receipt.items.forEach((item) => {
      let row = `${item.name}  ${item.price.toLocaleString()} сўм  ${item.quantity}  ${item.total.toLocaleString()} сўм`;
      if (receipt.paymentMethod === "credit") {
        row += `  ${item.creditMonth}  ${item.creditPercent}%  ${item.monthlyPayment.toLocaleString()} сўм`;
      }
      doc.text(row, 10, y);
      y += 10;
    });
    doc.setFontSize(12);
    doc.text(`Жами: ${receipt.total.toLocaleString()} сўм`, 10, y);
    y += 10;
    if (receipt.paymentMethod === "credit") {
      doc.text(`Фоиз билан жами: ${receipt.creditTotal.toLocaleString()} сўм`, 10, y);
      y += 10;
    }
    doc.text(`Тўланган сумма: ${receipt.amountPaid.toLocaleString()} сўм`, 10, y);
    y += 10;
    doc.text(`Қолган сумма: ${receipt.remainingBalance.toLocaleString()} сўм`, 10, y);
    y += 10;
    doc.text(`Қайтариш коди: ${receipt.returnCode}`, 10, y);
  ...
  return (
    <div style={{ marginLeft: "260px" }} className="space-y-0">
          ...
          <div className="bg-white rounded-xl max-w-md w-full">
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-4">
                ...
                <strong>{customerName || "Номаълум Мижоз"}</strong> учун
      ...
      <div style={{ display: "none" }}>
        <Receipt ref={receiptRef} order={cart} />
      </div>
    </div>
  );
};
