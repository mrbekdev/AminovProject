  ...
  async create(createTransactionDto: CreateTransactionDto): Promise<Transaction> {
    return this.prisma.$transaction(async (prisma) => {
      ...
      for (const item of createTransactionDto.items) {
        ...
        if (!product) throw new BadRequestException(`Invalid product ID: ${item.productId}`);
      ...
      for (const item of createTransactionDto.items) {
        ...
        if (createTransactionDto.type === 'SALE' || createTransactionDto.type === 'STOCK_ADJUSTMENT') {
          ...
          if (newQuantity < 0) throw new BadRequestException(`Cannot reduce stock below 0 for product: ${product.name}`);
          await prisma.product.update({
            where: { id: item.productId },
            data: { quantity: { set: newQuantity }, branchId: createTransactionDto.branchId || product.branchId },
          });
          ...
          await prisma.product.update({
            where: { id: item.productId },
            data: { quantity: { increment: item.quantity }, branchId: createTransactionDto.branchId || product.branchId },
          });
          await prisma.productStockHistory.create({
            data: {
              productId: item.productId,
