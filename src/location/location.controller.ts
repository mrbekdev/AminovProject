...
export class LocationController {
  ...
  async updateLocation(
    ...
    try {
      ...
      if (!this.locationService.validateCoordinates(latitude, longitude) || this.isTashkentLocation(latitude, longitude)) {
        throw new HttpException('Invalid coordinates provided or default Tashkent coordinates', HttpStatus.BAD_REQUEST);
      }
      ...
      // Broadcast to all connected clients (including operator panel)
      try {
        await this.locationGateway.broadcastLocationUpdate(location);
      } catch (broadcastError) {
        this.logger.error(
          `Failed to broadcast location update for user ${userId}:`,
          broadcastError,
        );
      }
  ...
  async getMyLocation(
    ...
    try {
      ...
      throw new HttpException(error.message || 'Failed to get location', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getOnlineUsers(
    ...
    try {
      ...
      this.logger.log(
        `Admin ${req.user.sub} retrieved ${users.length} online users${branchId ? ` for branch ${branchId}` : ''}`,
      );
  ...
  async getLocationStats(
    ...
    try {
      ...
      throw new HttpException(error.message || 'Failed to get location statistics', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async cleanupOfflineUsers(
    ...
    try {
      ...
      throw new HttpException(error.message || 'Failed to cleanup offline locations', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}